...
export default function LoginPage() {
  ...
  useEffect(() => {
    const fetchStoreData = async () => {
      ...
      // Fetch store data using the store URL
      try {
        const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/v1/api/stores/url/${storeUrl}`);
        const storeData = response.data;
        setStoreLogo(storeData.storeLogo); // Update state with the store logo URL
      } catch (error) {
